...
or prevent inlining of all methods of a referenced mapper using the `NoExpressionInlining` property
on the `UseStaticMapper` / `UseMapper` attribute.
...
[Mapper]
// highlight-start
[UseStaticMapper(typeof(EnumMapper), NoExpressionInlining = true)]
// highlight-end
public static partial class Mapper
{
    public static partial IQueryable<CarDto> ProjectToDto(this IQueryable<Car> q);
